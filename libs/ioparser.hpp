...
public:
    ...
    virtual bool operator>> (std::ostream& ostr) const { return false; };  
    ...
    virtual bool operator>> (std::string& str) const { std::ostringstream os; bool rval=(bool) (os << (*this)); str=os.str(); return rval; }
    virtual bool operator<< (const std::string& str) { std::istringstream is(str); return (bool) ((*this) << is); }
...
template <class U> 
inline bool read_udt(std::istream& istr, U& rval, unsigned long& rflags)
{
    istr.clear(); istr >> rval; 
    rflags|=iff_set;
    if (istr.good()) { rflags &=(~iff_nonvalid); return true;} else { rflags|=iff_nonvalid;  return false; } 
}
template<class U> 
inline bool write_udt(std::ostream& ostr, const U& rval, const std::string rname) 
{ ostr << rname << "\t"  << rval <<"\n";  return true;}
#define __MK_IT_IOFIELD(type) \
  template<> inline bool toolbox::IField<type>::operator<<(std::istream& istr)     \
  { return toolbox::read_udt(istr, value, flags); } \
  template<> inline bool toolbox::IField<type>::operator>>(std::ostream& ostr) const  \
  { return toolbox::write_udt(ostr, value, name); } \
  template<> inline bool toolbox::IField<const type>::operator>>(std::ostream& ostr) const \
  { return toolbox::write_udt(ostr, value, name); } 
...
template<> inline bool IField<bool>::operator<<(std::istream& istr)
{ 
    std::string val;
...
template<> inline bool IField<bool>::operator>>(std::ostream& ostr) const       { ostr<<name<<" "<<(value?"true":"false")<<"\n"; return true;}
template<> inline bool IField<const bool>::operator>>(std::ostream& ostr) const { ostr<<name<<" "<<(value?"true":"false")<<"\n"; return true;}
...
template<typename T>
bool IField<std::valarray<T> >::operator<<(std::istream& istr) 
{ 
...
template<typename T>
bool IField<std::valarray<T> >::operator>>(std::ostream& ostr) const
{ 
...
template<typename T>
bool IField<const std::valarray<T> >::operator>>(std::ostream& ostr) const
{ 
    ostr << name << " \t"<< value.size()<<"\n";
    for (unsigned long i=0; i<value.size(); ++i) ostr<<value[i]<<" ";
    ostr<<"\n";
...
template<typename T>
bool IField<std::vector<T> >::operator<<(std::istream& istr) 
{ 
    ...
    if (word=="{")
    {
        ...
        while (istr.good()) 
        {
            ...
            {
            case '}': 
                return true; break; //finished reading!
            case ' ':
            case '\t':
            case '\n':
                continue; //go on
...
template<typename T>
bool IField<std::vector<T> >::operator>>(std::ostream& ostr) const
{ 
    ostr << name << " \t"<< value.size()<<"\n";
    for (unsigned long i=0; i<value.size(); ++i) ostr<<value[i]<<" ";
    ostr<<"\n";
...
template<typename T>
bool IField<const std::vector<T> >::operator>>(std::ostream& ostr) const
{ 
    ostr << name << " \t"<< value.size()<<"\n";
    for (unsigned long i=0; i<value.size(); ++i) ostr<<value[i]<<" ";
    ostr<<"\n";
...
bool IField<std::map<K,T> >::operator<<(std::istream& istr) 
{ 
    //reads number of elements or braces starting the vector
    unsigned long nel;
    std::stringstream ls; 
...
template<class K, class T>
bool IField<std::map<K,T> >::operator>>(std::ostream& ostr) const
{
...
template<typename K,typename T>
bool IField<const std::map<K,T> >::operator>>(std::ostream& ostr) const
{ 
